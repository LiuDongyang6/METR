# This file is modified from the evit.py
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, mid_head_layers=(3,6,9),
                 tokens=None):
        """
        Args:
        ...
        self.mid_head_layers = mid_head_layers
        def create_mid_head():
        ...
        self.mid_heads = nn.ModuleDict({str(i): create_mid_head() for i in self.mid_head_layers})
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if idx is not None:
                idxs.append(idx)
            if i in self.mid_head_layers:
                mid_out = self.mid_heads[str(i)](x[:, :1])[:, 0]
                l_mid_out.append(mid_out)

        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), left_tokens, idxs, l_mid_out
        else:
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        if tokens is None:
            tokens = self.tokens
        x, _, idxs, mid_result = self.forward_features(x, keep_rate, tokens, get_idx)
...
# -------------------------------------------------------------
# EViT models with DiffRate-searched token drop schedule
@register_model
def deit_small_patch16_diffrate2dot3(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    tokens = [196, 179, 155, 126, 108, 97, 79, 65, 51, 36, 30, 2]
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, tokens=tokens)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def deit_small_patch16_diffrate2dot9(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    tokens = [196, 193, 175, 155, 140, 132, 120, 106, 87, 63, 55, 2]
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, keep_rate=keep_rate, tokens=tokens)
    model_kwargs.update(kwargs)
    model = _create_evit('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

# Modified from the main.py file in the root directory of the EViT project
...
from metr.engine import train_one_epoch, evaluate, visualize_mask
from losses import DistillationLoss
from samplers import RASampler
from metr import metr_evit
import utils
...
def get_args_parser():
    ...
    # multi exit
    parser.add_argument('--mid_loss_weight', default=0.15, type=float, help='weight for multi-exit loss')
    parser.add_argument('--mid_kd_weight', default=0.15, type=float, help='weight for multi-exit self-distillation loss')
    parser.add_argument('--mid_kd_tau', default=1.0, type=float, help='mid kd temperature')
    parser.add_argument('--mid_head_layers', default='(3, 6, 9)', type=str,
                        help='layer indices for multi-exit heads')
    return parser
...
def main(args):
    ...
    model = create_model(
        ...
        mid_head_layers=eval(args.mid_head_layers)
    )
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, args.base_keep_rate)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % test_interval == 0 or epoch == args.epochs - 1:
            ...
            if is_best:  # save_best
                checkpoint_paths = [output_dir / f'checkpoint_best{epoch}.pth']
                for checkpoint_path in checkpoint_paths:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'epoch': epoch,

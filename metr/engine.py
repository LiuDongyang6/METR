# modified from engine.py in the EViT root directory
...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            outputs, mid_outputs = model(samples, keep_rate)
            loss = criterion(samples, outputs, targets)
            mid_loss = sum([criterion(samples, _, targets) for _ in mid_outputs])
            mid_kd_loss = sum([utils.loss_kl(_, outputs, args.mid_kd_tau) for _ in mid_outputs])

        loss_value = loss.item()
        mid_loss_val = mid_loss.item()
        mid_kd_loss_val = mid_kd_loss.item()
        ...
        total_loss = loss + mid_loss * args.mid_loss_weight + mid_kd_loss * args.mid_kd_weight
        ...
        metric_logger.update(final_loss=loss_value)
        metric_logger.update(mid_loss=mid_loss_val)
        metric_logger.update(mid_kd=mid_kd_loss_val)
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(kr=keep_rate)

        if torch.distributed.get_rank() == 0 and it % log_interval == 0:
            writer.add_scalar('final_loss', loss_value, it)
            writer.add_scalar('mid_loss', mid_loss_val, it)
            writer.add_scalar('mid_kd', mid_kd_loss_val, it)
...
def evaluate(data_loader, model, device, keep_rate=None):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            output, mid_outputs = model(images, keep_rate)
            loss = criterion(output, target)

        acc1, acc5 = accuracy(output, target, topk=(1, 5))
        mid_accs = [accuracy(_, target, topk=(1,5)) for _ in mid_outputs]

        batch_size = images.shape[0]
        metric_logger.update(loss=loss.item())
        metric_logger.meters['acc1'].update(acc1.item(), n=batch_size)
        metric_logger.meters['acc5'].update(acc5.item(), n=batch_size)
        for i, _ in enumerate(mid_accs):
            mid_acc1, mid_acc5 = _
            metric_logger.meters[f'acc1_{i}'].update(mid_acc1.item(), n=batch_size)
            metric_logger.meters[f'acc5_{i}'].update(mid_acc5.item(), n=batch_size)
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
